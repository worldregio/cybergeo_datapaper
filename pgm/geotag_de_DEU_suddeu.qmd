---
title: "Geographical tags  - Suddeutsche Zeitung"
...
df<-readRDS("../corpus_raw/de_DEU_suddeu.RDS")
...
# Fin de textes à couper

# textes à supprimer
x$text<-str_replace_all(x$text,pattern = "STADT, LAND, LEUTE\\.","")
x$text<-str_replace_all(x$text,pattern = "^KOMMENTAR; ","")
x$text<-str_replace_all(x$text,pattern = "^LEUTE; ","")
x$text<-str_replace_all(x$text,pattern = "^LEUTE: ","")
x$text<-str_replace_all(x$text,pattern = "^LEUTE. ","")
x$text<-str_replace_all(x$text,pattern = "^Leute; ","")
x$text<-str_replace_all(x$text,pattern = "^Leute: ","")
x$text<-str_replace_all(x$text,pattern = "^PROFIL; ","")
x$text<-str_replace_all(x$text,pattern = "^Profil; ","")
x$text<-str_replace_all(x$text,pattern = "^BESTER DINGE; ","")
x$text<-str_replace_all(x$text,pattern = "^KURZKRITIK; ","")
x$text<-str_replace_all(x$text,pattern = "^STADT; ","")
x$text<-str_replace_all(x$text,pattern = "^STADT: ","")
x$text<-str_replace_all(x$text,pattern = "^STADT\\. ","")
x$text<-str_replace_all(x$text,pattern = "^HEUTE; ","")
x$text<-str_replace_all(x$text,pattern = "^HEUTE: ","")
x$text<-str_replace_all(x$text,pattern = "^HEUTE\\. ","")
x$text<-str_replace_all(x$text,pattern = "^REDEN WIR ÜBER; ","")
x$text<-str_replace_all(x$text,pattern = "^HEUTE; ","")
x$text<-str_replace_all(x$text,pattern = "^DEBATTE; ","")
x$text<-str_replace_all(x$text,pattern = "^DEBATTE: ","")
x$text<-str_replace_all(x$text,pattern = "^DEBATTE@SZ: ","")
...
extract_tags <- function(qd = qd,                       # the corpus of interest
                         lang = "de",                   # the language to be used
...
```{r}
td<-tidy(qd)
saveRDS(td,"../corpus_geo/de_DEU_suddeu_geo.RDS")
```
...
write.table(res,"../test/de_DEU_suddeu_geo_test.csv", row.names = F, sep=";")
```
