---
title: "Geographical tags - Le Figaro"
...
df<-readRDS("../corpus_raw/fr_FRA_figaro.RDS")
...
x$text<-str_replace(x$text,pattern = "Articles en rapport.*","")
x$text<-str_replace(x$text,pattern = "LIRE AUSSI.*","")
x$text<-str_replace(x$text,pattern = " LIRE AUSSI.*","")
x$text<-str_replace(x$text,pattern = "Lire aussi.*","")
x$text<-str_replace(x$text,pattern = " Lire aussi.*","")
x$text<-str_replace(x$text,pattern = "Lire la suite.*","")
x$text<-str_replace(x$text,pattern = " Lire la suite.*","")

# Débuts de texte à supprimer
x$text<-str_replace(x$text,pattern="^[[:upper:]│[:space:]]{5,}-[:space:]","")
x$text<-str_replace(x$text,pattern="^[[:upper:]│[:space:]]{5,}-","")
x$text<-str_replace(x$text,pattern="^[[:upper:]│[:space:]]{5,}/","")
x$text<-str_replace(x$text,pattern="^LA CHRONIQUE D'","") 
x$text<-str_replace(x$text,pattern="^VIDÉO - ","") 
x$text<-str_replace(x$text,pattern="^VIDÉO ","") 
x$text<-str_replace(x$text,pattern="VIDÉO","") 
x$text<-str_replace(x$text,pattern="POINT DE GUILLAUME TABARD -","") 
x$text<-str_replace(x$text,pattern="POINT DE GUILLAUME TABARD","")
x$text<-str_replace(x$text,pattern="POINT -","") 
x$text<-str_replace(x$text,pattern="POINT-","") 
...
```{r}
td<-tidy(qd)
saveRDS(td,"../corpus_geo/fr_FRA_figaro_geo.RDS")
```
